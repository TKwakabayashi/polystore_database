...
// execProjection は Projection のシンク実装。子を全件実行し、全 [] Record に対して
// unit ごとのプロパティを一括取得・射影し、最後にソート・リミットを適用する。
func (p *Processor) execProjection(o *plan.Projection) ([]map[string]interface{}, error) {
	...
	// C. 射影
	results := make([]map[string]interface{}, 0, len(in))
	for _, r := range in {
		results = append(results, projectRow([]string(r), o.Items, aliasToSlot, cache))
	}
	p.recordOp(step, "Projection", time.Since(start), len(in), len(results))

	final := applySortAndLimit(o, results)
	p.results = final
	return final, nil
}
